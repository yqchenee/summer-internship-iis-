...
int main(int argc, const char** argv) 
{
    ...
    for( int i = 3 ; i < argc ; i++ ) {
        ...
        string   businessFile , k_userFile , outFile ;

        string queryFile = "query_input/" ;
        queryFile.append( argv[i] ) ;
        if( !MG.readQueryFile( queryFile , Time , Q_user , businessFile , k_userFile , outFile , threshold_dist )) 
            continue ;
        ...
        // input k_userFile
        vector<unsigned> k_user ;
        if( !inputFile_to_vector( k_userFile , k_user ) ) continue ;
        ...
        if( !MG.calculate_final_query_score( outFile , given_business , k_user , Q_user , threshold_dist ) ) {
            cout << "query-k_user contains given User itself." << endl;
            continue ;
        }
